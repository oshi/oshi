...
* [#3681](https://github.com/oshi/oshi/pull/3681): Fix Linux `GpuStats#getGpuUtilization()` always returning -1 (and thus 0% to callers) for NVIDIA GPUs, because it only read the amdgpu/i915/xe sysfs paths and never queried NVML, unlike every other metric on that class - [@Krillsson](https://github.com/Krillsson).
