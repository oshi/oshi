...
* [#3555](https://github.com/oshi/oshi/pull/3555): Fix Linux graphics cards all reporting the same VRAM, DRM device and GPU metrics: request the stable `lspci -mm` machine-readable format so the `Slot:` field exists, stop discarding the PCI slot when entering a card record, and identify display controllers by PCI class `0x03` rather than a `VGA` substring match - [@Krillsson](https://github.com/Krillsson).
