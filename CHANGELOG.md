...
* [#3675](https://github.com/oshi/oshi/pull/3675): `CentralProcessor.getCurrentFreq()` on Apple Silicon can report the frequency each core actually ran at, rather than its cluster's nominal maximum, which never changes. Set `oshi.os.mac.cpu.frequency.ioreport` to `true` to weight each cluster's frequencies by the time the core spent at each of them since the previous call, read from the IOReport `CPU Stats / CPU Core Performance States` channels. It is opt-in because it holds a subscription to a private framework for the lifetime of the process - [@dbwiddis](https://github.com/dbwiddis).
