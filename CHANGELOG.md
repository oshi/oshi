...
* [#3614](https://github.com/oshi/oshi/pull/3614): Mark the `oshi.hardware` and `oshi.software.os` API `@NullMarked` with [jSpecify](https://jspecify.dev/), so every type in a signature is non-null unless annotated `@Nullable`, and enforce it in continuous integration with [NullAway](https://github.com/uber/NullAway). The `org.jspecify:jspecify` dependency is optional and compile-time only. Fixes `DisplayInfo.getPreferredResolution()`, `getSerialNo()`, `getModel()`, and `getProductSerialNumber()` returning `null` rather than an empty string for a synthesized display such as the Apple Silicon built-in panel - [@dbwiddis](https://github.com/dbwiddis).
