...
* [#3687](https://github.com/oshi/oshi/pull/3687): `GpuStats.getSharedMemoryUsed()` on Linux reports the amdgpu GTT total rather than always returning -1, and `GraphicsCard.getVRam()` on an amdgpu card reports the driver's own figure rather than the memory BAR size parsed from `lspci` or `lshw`. Several `GpuStats` metrics on Linux also now return the -1 sentinel when the sysfs file behind them is absent; they previously reported 0, so a card whose hwmon directory omits `power1_average` read as drawing 0.0 W - [@dbwiddis](https://github.com/dbwiddis).
