...
* [#3614](https://github.com/oshi/oshi/pull/3614): Mark the `oshi.hardware` and `oshi.software.os` API `@NullMarked` with [jSpecify](https://jspecify.dev/), so every type in a signature is non-null unless annotated `@Nullable`, and enforce it in continuous integration with [NullAway](https://github.com/uber/NullAway). The `org.jspecify:jspecify` dependency is optional and compile-time only. Fixes `DisplayInfo.getPreferredResolution()`, `getModel()`, and `getProductSerialNumber()` returning `null` rather than an empty string for a synthesized display such as the Apple Silicon built-in panel - [@dbwiddis](https://github.com/dbwiddis).
* [#3615](https://github.com/oshi/oshi/pull/3615): Mark `oshi.util` `@NullMarked`. Fixes an `ArrayIndexOutOfBoundsException` parsing a `/proc` statistics file containing an empty line. Adds `ParseUtil.getStringValueOrEmpty` and removes the unused `Util.isBlankOrUnknown` - [@dbwiddis](https://github.com/dbwiddis).
* [#3616](https://github.com/oshi/oshi/pull/3616): Report a disk or partition whose model, serial, or UUID reads as an empty string as `Constants.UNKNOWN`, matching every other unreadable value. Affects AIX disks whose `lscfg` serial field is blank, and the Linux and macOS equivalents - [@dbwiddis](https://github.com/dbwiddis).
