...
* [#3659](https://github.com/oshi/oshi/pull/3659): Setting `oshi.os.windows.hkeyperfdata` to `false` now also skips the registry when fetching thread counters, matching its documented behavior and the existing handling for processes. Threads previously read `HKEY_PERFORMANCE_DATA` regardless of the setting - [@dbwiddis](https://github.com/dbwiddis).
