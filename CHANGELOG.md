...
* [#3612](https://github.com/oshi/oshi/pull/3612): Stop substituting the exception message into log text in `ExceptionUtil` and the `ForeignFunctions` arena helpers, which appended it behind Error Prone's back at 200 further call sites. Two logs that had been silently losing their argument to that substitution now report it: the registry key whose subkeys could not be enumerated, and the performance-data object that could not be read. `ForeignFunctions.dlopenWithFlags` also now attaches the underlying failure as the cause of the `UnsatisfiedLinkError` it throws, rather than copying its message - [@dbwiddis](https://github.com/dbwiddis).
* [#3614](https://github.com/oshi/oshi/pull/3614): Document the nullability of the `oshi.hardware` and `oshi.software.os` API with [jSpecify](https://jspecify.dev/) annotations. Both packages are now `@NullMarked`, so every type in a signature is non-null unless annotated `@Nullable`; the handful of nullable members (such as `OperatingSystem.getProcess(int)`, `PowerSource.getManufactureDate()`, and the optional `filter`/`sort` process arguments) are annotated and documented as such. The annotations are enforced rather than advisory: [NullAway](https://github.com/uber/NullAway) now runs in continuous integration over the marked packages, so a signature not annotated `@Nullable` is a checked guarantee. The `org.jspecify:jspecify` dependency is compile-time metadata: declared `optional` in Maven, `requires static` in the module descriptor, and an optional OSGi import, so it does not reach a consumer's runtime classpath. This also fixes `DisplayInfo.getPreferredResolution()`, `getSerialNo()`, and `getModel()` returning `null` (rather than the documented empty string) for a synthesized display such as the Apple Silicon built-in panel, which also made `DisplayInfo.toString()` print `null`, and corrects the `OperatingSystem.getProcesses()` javadoc, which claimed the returned list could contain `null` elements and be sorted - [@dbwiddis](https://github.com/dbwiddis).
