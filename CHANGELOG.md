...
* [#3614](https://github.com/oshi/oshi/pull/3614), [#3615](https://github.com/oshi/oshi/pull/3615), [#3618](https://github.com/oshi/oshi/pull/3618), [#3619](https://github.com/oshi/oshi/pull/3619), [#3620](https://github.com/oshi/oshi/pull/3620), [#3621](https://github.com/oshi/oshi/pull/3621), [#3622](https://github.com/oshi/oshi/pull/3622), [#3623](https://github.com/oshi/oshi/pull/3623), [#3624](https://github.com/oshi/oshi/pull/3624), [#3625](https://github.com/oshi/oshi/pull/3625), [#3626](https://github.com/oshi/oshi/pull/3626), [#3627](https://github.com/oshi/oshi/pull/3627), [#3628](https://github.com/oshi/oshi/pull/3628), [#3629](https://github.com/oshi/oshi/pull/3629), [#3630](https://github.com/oshi/oshi/pull/3630), [#3631](https://github.com/oshi/oshi/pull/3631), [#3632](https://github.com/oshi/oshi/pull/3632), [#3633](https://github.com/oshi/oshi/pull/3633), [#3634](https://github.com/oshi/oshi/pull/3634), [#3635](https://github.com/oshi/oshi/pull/3635): Mark `oshi-common`, `oshi-core` and `oshi-core-ffm` `@NullMarked` with [jSpecify](https://jspecify.dev/) and enforce it in continuous integration with [NullAway](https://github.com/uber/NullAway), so every type in a signature is non-null unless annotated `@Nullable`. The `org.jspecify:jspecify` dependency is optional and compile-time only. Adds `ParseUtil.getStringValueOrEmpty` and removes the unused `Util.isBlankOrUnknown`. Fixes getters that returned `null` despite promising a string: `DisplayInfo`'s preferred resolution, model and product serial number, `SoundCard`'s driver version and codec, `UsbDevice`'s vendor and unique device ID, `ComputerSystem`, `Baseboard` and `Firmware` fields on AIX, Windows and the BSDs, an installed application's name, version and vendor on every platform, and the domain name on FreeBSD when the sysctl read fails. A macOS process whose passwd or group entry cannot be read now reports its numeric user and group id through the FFM implementation, matching JNA, rather than reporting neither; Linux now reports an unreadable ALSA driver version as `Constants.UNKNOWN` rather than the string `not available`. Fixes exceptions thrown on incomplete platform output: an empty line in a `/proc` statistics file, an AIX logical volume missing its type or partition count, a BSD `ps` line missing its process state, and a Solaris system with no SMBIOS section of a given type. Windows CPU ticks are no longer reported as all zero when an optional processor utility counter is unavailable - [@dbwiddis](https://github.com/dbwiddis).
