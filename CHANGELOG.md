...
* [#3614](https://github.com/oshi/oshi/pull/3614): Document the nullability of the `oshi.hardware` and `oshi.software.os` API with [jSpecify](https://jspecify.dev/) annotations. Both packages are now `@NullMarked`, so every type in a signature is non-null unless annotated `@Nullable`; the handful of nullable members (such as `OperatingSystem.getProcess(int)`, `PowerSource.getManufactureDate()`, and the optional `filter`/`sort` process arguments) are annotated and documented as such. The `org.jspecify:jspecify` dependency is compile-time metadata: declared `optional` in Maven, `requires static` in the module descriptor, and an optional OSGi import, so it does not reach a consumer's runtime classpath. This also fixes `DisplayInfo.getPreferredResolution()`, `getSerialNo()`, and `getModel()` returning `null` (rather than the documented empty string) for a synthesized display such as the Apple Silicon built-in panel, which also made `DisplayInfo.toString()` print `null`, and corrects the `OperatingSystem.getProcesses()` javadoc, which claimed the returned list could contain `null` elements and be sorted - [@dbwiddis](https://github.com/dbwiddis).
