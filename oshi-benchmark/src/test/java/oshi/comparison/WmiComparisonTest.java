...
class WmiComparisonTest {
    ...
    void testLogicalDiskUnfiltered() {
        ...
        WmiResult<LogicalDiskProperty> ffm = Win32LogicalDiskFFM.queryLogicalDisk(null, false);
                ...
                for (LogicalDiskProperty key : LogicalDiskProperty.values()) {
                    ...
                    switch (key) {
                        case NAME, DESCRIPTION, FILESYSTEM, VOLUMENAME, PROVIDERNAME -> assertThat(
                                WmiUtil.getString(ffm, key, fi)).as(desc).isEqualTo(WmiUtil.getString(jna, key, ji));
                        case DRIVETYPE -> assertThat(WmiUtil.getUint32(ffm, key, fi)).as(desc)
                                .isEqualTo(WmiUtil.getUint32(jna, key, ji));
                        case ACCESS -> assertThat(WmiUtil.getUint16(ffm, key, fi)).as(desc)
                                .isEqualTo(WmiUtil.getUint16(jna, key, ji));
                        case SIZE -> assertThat(WmiUtil.getUint64(ffm, key, fi)).as(desc)
                                .isEqualTo(WmiUtil.getUint64(jna, key, ji));
                        default -> assertWithinRatio(WmiUtil.getUint64(ffm, key, fi), WmiUtil.getUint64(jna, key, ji),
                                0.05, desc);
                    }
                }
            }
    ...
    void testLogicalDiskLocalOnly() {
        ...
        WmiResult<LogicalDiskProperty> ffm = Win32LogicalDiskFFM.queryLogicalDisk(null, true);
        ...
        for (String name : jnaByName.keySet()) {
            ...
            int driveType = WmiUtil.getUint32(ffm, LogicalDiskProperty.DRIVETYPE, fi);
    ...
    void testLogicalDiskNameFilter() {
        ...
        WmiResult<LogicalDiskProperty> ffm = Win32LogicalDiskFFM.queryLogicalDisk(firstName, false);
        ...
        assertThat(WmiUtil.getString(ffm, LogicalDiskProperty.NAME, 0)).as("LogicalDisk filtered NAME")
                .isEqualTo(firstName);
    }
    ...
    void testBaseBoard() {
        ...
        WmiResult<BaseBoardProperty> ffm = Win32BaseBoardFFM.queryBaseboardInfo();
        ...
        for (BaseBoardProperty p : BaseBoardProperty.values()) {
            assertThat(WmiUtil.getString(ffm, p, 0)).as("BaseBoard " + p).isEqualTo(WmiUtil.getString(jna, p, 0));
        }
    }
    ...
    void testBiosInfo() {
        ...
        WmiResult<BiosProperty> ffm = Win32BiosFFM.queryBiosInfo();
        ...
        for (BiosProperty p : BiosProperty.values()) {
            if (p == BiosProperty.RELEASEDATE) {
                assertThat(WmiUtil.getDateString(ffm, p, 0)).as("Bios " + p)
                        .isEqualTo(WmiUtil.getDateString(jna, p, 0));
            } else {
                assertThat(WmiUtil.getString(ffm, p, 0)).as("Bios " + p).isEqualTo(WmiUtil.getString(jna, p, 0));
            }
        }
    }
    ...
    void testBiosSerialNumber() {
        ...
        WmiResult<BiosSerialProperty> ffm = Win32BiosFFM.querySerialNumber();
        ...
        assertThat(WmiUtil.getString(ffm, BiosSerialProperty.SERIALNUMBER, 0)).as("BiosSerial SERIALNUMBER")
    ...
    void testComputerSystem() {
        ...
        WmiResult<ComputerSystemProperty> ffm = Win32ComputerSystemFFM.queryComputerSystem();
        ...
        for (ComputerSystemProperty p : ComputerSystemProperty.values()) {
            assertThat(WmiUtil.getString(ffm, p, 0)).as("ComputerSystem " + p).isEqualTo(WmiUtil.getString(jna, p, 0));
        }
    }
    ...
    void testComputerSystemProduct() {
        ...
        WmiResult<ComputerSystemProductProperty> ffm = Win32ComputerSystemProductFFM.queryIdentifyingNumberUUID();
        ...
        for (ComputerSystemProductProperty p : ComputerSystemProductProperty.values()) {
            assertThat(WmiUtil.getString(ffm, p, 0)).as("ComputerSystemProduct " + p)
    ...
    void testOperatingSystem() {
        ...
        WmiResult<OSVersionProperty> ffm = Win32OperatingSystemFFM.queryOsVersion();
        ...
        for (OSVersionProperty p : OSVersionProperty.values()) {
            switch (p) {
                case PRODUCTTYPE, SUITEMASK -> assertThat(WmiUtil.getUint32(ffm, p, 0)).as("OS " + p)
                        .isEqualTo(WmiUtil.getUint32(jna, p, 0));
                default -> assertThat(WmiUtil.getString(ffm, p, 0)).as("OS " + p)
                        .isEqualTo(WmiUtil.getString(jna, p, 0));
            }
        }
    }
    ...
    void testProcessor() {
        ...
        WmiResult<ProcessorIdProperty> ffm = Win32ProcessorFFM.queryProcessorId();
        ...
        for (int i = 0; i < jna.getResultCount(); i++) {
            assertThat(WmiUtil.getString(ffm, ProcessorIdProperty.PROCESSORID, i)).as("PROCESSORID [" + i + "]")
        ...
        WmiResult<BitnessProperty> ffmBit = Win32ProcessorFFM.queryBitness();
        ...
        for (int i = 0; i < jnaBit.getResultCount(); i++) {
            assertThat(WmiUtil.getUint16(ffmBit, BitnessProperty.ADDRESSWIDTH, i)).as("ADDRESSWIDTH [" + i + "]")
        ...
        WmiResult<VoltProperty> ffmVolt = Win32ProcessorFFM.queryVoltage();
        ...
        for (int i = 0; i < jnaVolt.getResultCount(); i++) {
            assertThat(WmiUtil.getUint16(ffmVolt, VoltProperty.CURRENTVOLTAGE, i)).as("CURRENTVOLTAGE [" + i + "]")
    ...
    void testPhysicalMemory() {
        ...
        WmiResult<PhysicalMemoryProperty> ffm = Win32PhysicalMemoryFFM.queryPhysicalMemory();
        ...
        for (String key : jnaByKey.keySet()) {
            ...
            for (int idx = 0; idx < jnaIndices.size(); idx++) {
                int ji = jnaIndices.get(idx);
                int fi = ffmIndices.get(idx);
                assertThat(WmiUtil.getString(ffm, PhysicalMemoryProperty.BANKLABEL, fi))
                        .as("BANKLABEL [" + key + "#" + idx + "]")
                        .isEqualTo(WmiUtil.getString(jna, PhysicalMemoryProperty.BANKLABEL, ji));
                assertThat(WmiUtil.getUint64(ffm, PhysicalMemoryProperty.CAPACITY, fi))
                        .as("CAPACITY [" + key + "#" + idx + "]")
                        .isEqualTo(WmiUtil.getUint64(jna, PhysicalMemoryProperty.CAPACITY, ji));
                assertThat(WmiUtil.getString(ffm, PhysicalMemoryProperty.MANUFACTURER, fi))
    ...
    void testVideoController() {
        ...
        WmiResult<VideoControllerProperty> ffm = Win32VideoControllerFFM.queryVideoController();
        ...
        for (String key : jnaByKey.keySet()) {
            ...
            for (int idx = 0; idx < jnaIndices.size(); idx++) {
                int ji = jnaIndices.get(idx);
                int fi = ffmIndices.get(idx);
                assertThat(WmiUtil.getString(ffm, VideoControllerProperty.NAME, fi)).as("VC NAME [" + key + "]")
                        .isEqualTo(WmiUtil.getString(jna, VideoControllerProperty.NAME, ji));
                assertThat(WmiUtil.getString(ffm, VideoControllerProperty.DRIVERVERSION, fi))
    ...
    void testPrinter() {
        ...
        WmiResult<PrinterProperty> ffm = Win32PrinterFFM.queryPrinters();
        ...
        for (String name : jnaByKey.keySet()) {
            ...
            for (int idx = 0; idx < jnaIndices.size(); idx++) {
                int ji = jnaIndices.get(idx);
                int fi = ffmIndices.get(idx);
                assertThat(WmiUtil.getString(ffm, PrinterProperty.DRIVERNAME, fi))
    ...
    void testWin32ProcessCommandLines() {
        ...
        WmiResult<CommandLineProperty> ffm = Win32ProcessFFM.queryCommandLines(currentPid);
        assertThat(ffm.getResultCount()).as("Win32Process commandLine count").isEqualTo(jna.getResultCount());
        if (jna.getResultCount() > 0) {
            assertThat(WmiUtil.getUint32(ffm, CommandLineProperty.PROCESSID, 0)).as("Win32Process PID")
                    .isEqualTo(WmiUtil.getUint32(jna, CommandLineProperty.PROCESSID, 0));
            assertThat(WmiUtil.getString(ffm, CommandLineProperty.COMMANDLINE, 0)).as("Win32Process commandLine")
    ...
    void testWin32ProcessXP() {
        ...
        WmiResult<ProcessXPProperty> ffm = Win32ProcessFFM.queryProcesses(currentPid);
    ...
    void testLhmSensors() {
        ...
        WmiResult<LhmSensorProperty> ffm = LhmSensorFFM.querySensors(null, null);
    ...
    void testLhmGpuHardware() {
        ...
        WmiResult<LhmHardwareProperty> ffm = LhmSensorFFM.queryGpuHardware();
    ...
    void testFanSpeed() {
        ...
        WmiResult<SpeedProperty> ffm = Win32FanFFM.querySpeed();
    ...
    void testMSAcpiTemperature() {
        ...
        WmiResult<TemperatureProperty> ffm = MSAcpiThermalZoneTemperatureFFM.queryCurrentTemperature();
    ...
    private static <T extends Enum<T>> Map<String, List<Integer>> buildFfmStringMap(WmiResult<T> result, T keyProp) {
        Map<String, List<Integer>> map = new HashMap<>();
        for (int i = 0; i < result.getResultCount(); i++) {
            map.computeIfAbsent(WmiUtil.getString(result, keyProp, i), k -> new ArrayList<>()).add(i);
        }
