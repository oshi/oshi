...
public class LinuxProcess extends AbstractProcess {
     ...
     */
    private static long MS_PER_JIFFIE = 1L;

    /**
     * Boot time in MS.
     */
    private static long BOOT_TIME = 0L;
    ...
    private static void init() {
        ...
        // process in seconds. This takes advantage of the fact that ps does all
        // the heavy lifting of sorting out HZ internally.
        ...
        BOOT_TIME = System.currentTimeMillis() - (long) (1000 * startTimeSecsSinceBoot);

        // Timeline:
        // BOOT|<----jiffies---->|<----etime---->|NOW
        // BOOT|<------------uptime------------->|NOW
        // By subtracting etime (secs) from uptime (secs) we get uptime when the
        // process was started. Subtract from current time to get this process
        ...
        MS_PER_JIFFIE = (long) (1000 * startTimeSecsSinceBoot / youngestJiffies + 0.5f);
    }
    ...
    public LinuxProcess(String name, String path, char state, int processID, int parentProcessID, int threadCount,
        ...
        this.kernelTime = kernelTime * MS_PER_JIFFIE;
        this.userTime = userTime * MS_PER_JIFFIE;
        this.startTime = startTime * MS_PER_JIFFIE + BOOT_TIME;
