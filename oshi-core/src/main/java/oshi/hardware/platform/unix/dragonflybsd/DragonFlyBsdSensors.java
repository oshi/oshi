 ...
 */
@ThreadSafe
final class DragonFlyBsdSensors extends AbstractSensors {
    ...
    private static double queryKldloadCoretemp() {
        ...
        try (CloseableSizeTByReference size = new CloseableSizeTByReference(DragonFlyBsdLibc.INT_SIZE)) {
            int cpu = 0;
            double sumTemp = 0d;
            try (Memory p = new Memory(size.longValue())) {
                while (0 == DragonFlyBsdLibc.INSTANCE.sysctlbyname(String.format(Locale.ROOT, name, cpu), p, size, null,
                        size_t.ZERO)) {
