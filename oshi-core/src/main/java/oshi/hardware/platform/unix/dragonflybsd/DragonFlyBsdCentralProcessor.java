...
import oshi.jna.platform.unix.DragonFlyBsdLibc.CpTime;
 ...
 */
@ThreadSafe
final class DragonFlyBsdCentralProcessor extends AbstractCentralProcessor {
    ...
    public long[] querySystemCpuLoadTicks() {
        ...
        try (CpTime cpTime = new CpTime()) {
            ...
            ticks[TickType.USER.getIndex()] = cpTime.cpu_ticks[DragonFlyBsdLibc.CP_USER];
            ticks[TickType.NICE.getIndex()] = cpTime.cpu_ticks[DragonFlyBsdLibc.CP_NICE];
            ticks[TickType.SYSTEM.getIndex()] = cpTime.cpu_ticks[DragonFlyBsdLibc.CP_SYS];
            ticks[TickType.IRQ.getIndex()] = cpTime.cpu_ticks[DragonFlyBsdLibc.CP_INTR];
            ticks[TickType.IDLE.getIndex()] = cpTime.cpu_ticks[DragonFlyBsdLibc.CP_IDLE];
        }
        return ticks;
    }
    ...
    public double[] getSystemLoadAverage(int nelem) {
        ...
        int retval = DragonFlyBsdLibc.INSTANCE.getloadavg(average, nelem);
        if (retval < nelem) {
        ...
        try (Memory p = new Memory(arraySize);
            ...
            for (int cpu = 0; cpu < getLogicalProcessorCount(); cpu++) {
                ticks[cpu][TickType.USER.getIndex()] = p
                        .getLong(CPTIME_SIZE * cpu + DragonFlyBsdLibc.CP_USER * DragonFlyBsdLibc.UINT64_SIZE); // lgtm
                ticks[cpu][TickType.NICE.getIndex()] = p
                        .getLong(CPTIME_SIZE * cpu + DragonFlyBsdLibc.CP_NICE * DragonFlyBsdLibc.UINT64_SIZE); // lgtm
                ticks[cpu][TickType.SYSTEM.getIndex()] = p
                        .getLong(CPTIME_SIZE * cpu + DragonFlyBsdLibc.CP_SYS * DragonFlyBsdLibc.UINT64_SIZE); // lgtm
                ticks[cpu][TickType.IRQ.getIndex()] = p
                        .getLong(CPTIME_SIZE * cpu + DragonFlyBsdLibc.CP_INTR * DragonFlyBsdLibc.UINT64_SIZE); // lgtm
                ticks[cpu][TickType.IDLE.getIndex()] = p
                        .getLong(CPTIME_SIZE * cpu + DragonFlyBsdLibc.CP_IDLE * DragonFlyBsdLibc.UINT64_SIZE); // lgtm
            }
        }
        return ticks;
    }
    ...
    public long queryContextSwitches() {
        ...
        size_t.ByReference size = new size_t.ByReference(new size_t(DragonFlyBsdLibc.INT_SIZE));
        try (Memory p = new Memory(size.longValue())) {
            if (0 != DragonFlyBsdLibc.INSTANCE.sysctlbyname(name, p, size, null, size_t.ZERO)) {
                return 0L;
            }
