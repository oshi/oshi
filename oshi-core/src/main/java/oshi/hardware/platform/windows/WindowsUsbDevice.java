...
public class WindowsUsbDevice extends AbstractUsbDevice {
    ...
    static {
        ...
        for (int i = 0; i < usbController.getResultCount(); i++) {
            controllerDeviceIdList.add((String) usbController.getString(USBControllerProperty.PNPDEVICEID, i));
        }
    }
    ...
    private static void updateDeviceCache() {
        ...
        if (!devicesToAdd.isEmpty()) {
            ...
            for (int i = 0; i < pnpEntity.getResultCount(); i++) {
                String pnpDeviceID = (String) pnpEntity.getString(PnPEntityProperty.PNPDEVICEID, i);
                String name = (String) pnpEntity.getString(PnPEntityProperty.NAME, i);
                String vendor = (String) pnpEntity.getString(PnPEntityProperty.MANUFACTURER, i);
            ...
            for (int i = 0; i < serialNumber.getResultCount(); i++) {
                String pnpDeviceID = (String) serialNumber.getString(DiskDriveProperty.PNPDEVICEID, i);
                if (usbDeviceCache.containsKey(pnpDeviceID)) {
                    ...
                    device.serialNumber = ParseUtil
                            .hexStringToString((String) serialNumber.getString(DiskDriveProperty.SERIALNUMBER, i));
                }
            }
