...
/**
 * A <code>Pointer</code> to memory obtained from the native heap via a call to
 * <code>malloc</code>.
 *
 * <p>
 * In some cases it might be necessary to use memory obtained from
 * <code>malloc</code>. For example, <code>Memory</code> helps accomplish the
 * following idiom:
 *
 * <pre>
 ...
 * @see Pointer
 */
public class CleanerMemory extends Pointer implements Closeable {
    /**
     * Keep track of all allocated memory so we can dispose of it before unloading.
     */
    private static final Map<Long, Reference<CleanerMemory>> allocatedMemory = new ConcurrentHashMap<Long, Reference<CleanerMemory>>();

    private static final WeakCleanerMemoryHolder buffers = new WeakCleanerMemoryHolder();
    ...
    public static void disposeAll() {
        ...
        Collection<Reference<CleanerMemory>> refs = new LinkedList<Reference<CleanerMemory>>(allocatedMemory.values());
        for (Reference<CleanerMemory> r : refs) {
            CleanerMemory m = r.get();
            if (m != null) {
                m.close();
            }
        }
    ...
    /**
     * Provide a view into the original memory. Keeps an implicit reference to the
     * original to prevent GC.
     */
    private class SharedMemory extends CleanerMemory {
        public SharedMemory(long offset, long size) {
            this.size = size;
            this.peer = CleanerMemory.this.peer + offset;
        }
        ...
        protected void boundsCheck(long off, long sz) {
            CleanerMemory.this.boundsCheck(this.peer - CleanerMemory.this.peer + off, sz);
        }

        @Override
        public String toString() {
            return super.toString() + " (shared from " + CleanerMemory.this.toString() + ")";
        }
    }
     ...
     */
    public CleanerMemory(long size) {
        super(0);
        this.size = size;
        if (size <= 0) {
        ...
        allocatedMemory.put(peer, new WeakReference<CleanerMemory>(this));
        cleanable = Cleaner.getCleaner().register(this, new MemoryDisposer(peer));
    }

    protected CleanerMemory() {
        super(0);
    }

    /**
     * Provide a view of this memory using the given offset as the base address. The
     * returned {@link Pointer} will have a size equal to that of the original minus
     * the offset.
     *
     ...
     * bounds-limited with the given size. Maintains a reference to the original
     * {@link CleanerMemory} object to avoid GC as long as the shared memory is
     * referenced.
     *
     * @throws IndexOutOfBoundsException
     *             if the requested memory is outside the allocated bounds.
     ...
     *
     * @param byteBoundary
     *            Align memory to this number of bytes; should be a power of two.
     * @throws IndexOutOfBoundsException
     *             if the requested alignment can not be met.
     * @throws IllegalArgumentException
     *             if the requested alignment is not a positive power of two.
     */
    public CleanerMemory align(int byteBoundary) {
        if (byteBoundary <= 0) {
        ...
        for (int i = 0; i < 32; i++) {
            if (byteBoundary == (1 << i)) {
                ...
                if ((peer & mask) != peer) {
                    ...
                    return (CleanerMemory) share(newPeer - peer, newSize);
                }
                return this;
            }
        }
     ...
     * <code>Pointer.read</code>. But this method performs a bounds checks to ensure
     * that the indirection does not cause memory outside the <code>malloc</code>ed
    ...
    /**
     * Indirect the native pointer to <code>malloc</code> space, a la
     * <code>Pointer.write</code>. But this method performs a bounds checks to
    ...
    /**
     * Indirect the native pointer to <code>malloc</code> space, a la
     * <code>Pointer.getByte</code>. But this method performs a bounds checks to
     * ensure that the indirection does not cause memory outside the
     * <code>malloc</code>ed space to be accessed.
     *
    ...
    /**
     * Indirect the native pointer to <code>malloc</code> space, a la
     * <code>Pointer.getShort</code>. But this method performs a bounds checks to
     * ensure that the indirection does not cause memory outside the
     * <code>malloc</code>ed space to be accessed.
     *
    ...
    /**
     * Indirect the native pointer to <code>malloc</code> space, a la
     * <code>Pointer.getInt</code>. But this method performs a bounds checks to
     * ensure that the indirection does not cause memory outside the
     * <code>malloc</code>ed space to be accessed.
     *
    ...
    /**
     * Indirect the native pointer to <code>malloc</code> space, a la
     * <code>Pointer.getLong</code>. But this method performs a bounds checks to
     * ensure that the indirection does not cause memory outside the
     * <code>malloc</code>ed space to be accessed.
     *
    ...
    /**
     * Indirect the native pointer to <code>malloc</code> space, a la
     * <code>Pointer.getFloat</code>. But this method performs a bounds checks to
     * ensure that the indirection does not cause memory outside the
     * <code>malloc</code>ed space to be accessed.
     *
    ...
    /**
     * Indirect the native pointer to <code>malloc</code> space, a la
     * <code>Pointer.getDouble</code>. But this method performs a bounds check to
     * ensure that the indirection does not cause memory outside the
     * <code>malloc</code>ed space to be accessed.
     *
    ...
    /**
     * Indirect the native pointer to <code>malloc</code> space, a la
     * <code>Pointer.getPointer</code>. But this method performs a bounds checks to
     * ensure that the indirection does not cause memory outside the
     * <code>malloc</code>ed space to be accessed.
     *
    ...
    /**
     * Get a ByteBuffer mapped to a portion of this memory. We keep a weak reference
     * to all ByteBuffers provided so that this memory object is not GC'd while
     * there are still implicit outstanding references to it (it'd be nice if we
     * could attach our own reference to the ByteBuffer, but the VM generates the
     * object so we have no control over it).
     *
     * @param offset
    ...
    /**
     * Indirect the native pointer to <code>malloc</code> space, a la
     * <code>Pointer.setByte</code>. But this method performs a bounds checks to
     * ensure that the indirection does not cause memory outside the
     * <code>malloc</code>ed space to be accessed.
     *
     * @see Pointer#setByte
    ...
    /**
     * Indirect the native pointer to <code>malloc</code> space, a la
     * <code>Pointer.setChar</code>. But this method performs a bounds checks to
     * ensure that the indirection does not cause memory outside the
     * <code>malloc</code>ed space to be accessed.
     *
     * @see Pointer#setChar
    ...
    /**
     * Indirect the native pointer to <code>malloc</code> space, a la
     * <code>Pointer.setShort</code>. But this method performs a bounds checks to
     * ensure that the indirection does not cause memory outside the
     * <code>malloc</code>ed space to be accessed.
     *
    ...
    /**
     * Indirect the native pointer to <code>malloc</code> space, a la
     * <code>Pointer.setInt</code>. But this method performs a bounds checks to
     * ensure that the indirection does not cause memory outside the
     * <code>malloc</code>ed space to be accessed.
     *
     * @see Pointer#setInt
    ...
    /**
     * Indirect the native pointer to <code>malloc</code> space, a la
     * <code>Pointer.setLong</code>. But this method performs a bounds checks to
     * ensure that the indirection does not cause memory outside the
     * <code>malloc</code>ed space to be accessed.
     *
     * @see Pointer#setLong
    ...
    /**
     * Indirect the native pointer to <code>malloc</code> space, a la
     * <code>Pointer.setFloat</code>. But this method performs a bounds checks to
     * ensure that the indirection does not cause memory outside the
     * <code>malloc</code>ed space to be accessed.
     *
    ...
    /**
     * Indirect the native pointer to <code>malloc</code> space, a la
     * <code>Pointer.setDouble</code>. But this method performs a bounds checks to
     * ensure that the indirection does not cause memory outside the
     * <code>malloc</code>ed space to be accessed.
     *
    ...
    /**
     * Indirect the native pointer to <code>malloc</code> space, a la
     * <code>Pointer.setPointer</code>. But this method performs a bounds checks to
     * ensure that the indirection does not cause memory outside the
     * <code>malloc</code>ed space to be accessed.
     *
    ...
    public void setString(long offset, String value, String encoding) {
        try {
            boundsCheck(offset, value.getBytes(encoding).length + 1L);
     ...
     * backed by this memory object. The intention is to prevent premature GC of the
     * Memory object.
     *
     * @param target
     *            Pointer to check
     * @return {@code target} if target does not point into the region covered by
     *         this memory object, a newly {@code SharedMemory} object, if the
     *         pointer points to memory backed by this Memory object.
     */
    private Pointer shareReferenceIfInBounds(Pointer target) {
        if (target == null) {
            return null;
        }
        long offset = Pointer.nativeValue(target) - this.peer;
