...
public interface Ole32 extends com.sun.jna.platform.win32.Ole32 {
     ...
     * does. CoTaskMemAlloc uses the default allocator to allocate a memory
     * block in the same way that IMalloc::Alloc does. It is not necessary to
     * call the CoGetMalloc function before calling CoTaskMemAlloc.
     * <p>
     * The initial contents of the returned memory block are undefined - there
     * is no guarantee that the block has been initialized. The allocated block
     * may be larger than cb bytes because of the space required for alignment
     * and for maintenance information.
     * </p>
     * If cb is 0, CoTaskMemAlloc allocates a zero-length item and returns a
     * valid pointer to that item. If there is insufficient memory available,
     * CoTaskMemAlloc returns NULL. Applications should always check the return
     * value from this function, even when requesting small amounts of memory,
     * because there is no guarantee that the memory will be allocated.
     * 
     * @param cb
     *            The size of the memory block to be allocated, in bytes.
     * @return If the function succeeds, it returns the allocated memory block.
     *         Otherwise, it returns NULL.
     */
    Pointer CoTaskMemAlloc(long cb);

    /**
     * Changes the size of a previously allocated block of task memory. This
     * function changes the size of a previously allocated memory block in the
     * same way that IMalloc::Realloc does. It is not necessary to call the
     * CoGetMalloc function to get a pointer to the OLE allocator before calling
     * CoTaskMemRealloc.
     * <p>
     * The pv parameter points to the beginning of the memory block. If pv is
     * NULL, CoTaskMemRealloc allocates a new memory block in the same way as
     * the CoTaskMemAlloc function. If pv is not NULL, it should be a pointer
     * returned by a prior call to CoTaskMemAlloc.
     * </p>
     * <p>
     * The cb parameter specifies the size of the new block. The contents of the
     * block are unchanged up to the shorter of the new and old sizes, although
     * the new block can be in a different location. Because the new block can
     * be in a different memory location, the pointer returned by
     * CoTaskMemRealloc is not guaranteed to be the pointer passed through the
     * pv argument. If pv is not NULL and cb is 0, then the memory pointed to by
     * pv is freed.
     * </p>
     * CoTaskMemRealloc returns a void pointer to the reallocated (and possibly
     * moved) memory block. The return value is NULL if the size is 0 and the
     * buffer argument is not NULL, or if there is not enough memory available
     * to expand the block to the specified size. In the first case, the
     * original block is freed; in the second case, the original block is
     * unchanged. The storage space pointed to by the return value is guaranteed
     ...
     * 
     * @param pv
     *            A pointer to the memory block to be reallocated. This
     *            parameter can be NULL.
     * @param cb
     *            The size of the memory block to be reallocated, in bytes. This
     *            parameter can be 0.
     * @return If the function succeeds, it returns the reallocated memory
     ...
     * {@link #CoTaskMemAlloc} or {@link #CoTaskMemRealloc} function. The
     * function uses the default OLE allocator. The number of bytes freed equals
     * the number of bytes that were originally allocated or reallocated. After
     * the call, the memory block pointed to by pv is invalid and can no longer
     * be used.
     * 
     * @param pv
     *            A pointer to the memory block to be freed. If this parameter
     ...
     * @return status
     * @see <a href=
     *      "http://msdn.microsoft.com/en-us/library/windows/desktop/ms693395%28v=vs.85%29.aspx">MSDN</a>
     * 
     */
     ...
     * @return status
     * @see <a href=
     *      "http://msdn.microsoft.com/en-us/library/windows/desktop/ms678542%28v=vs.85%29.aspx">MSDN</a>
     */
    HRESULT CreateBindCtx(DWORD reserved, PointerByReference ppbc);

    /**
     * Determines whether a remote object is connected to the corresponding
     * in-process object.
     * 
     * {@code
     ...
     * @see <a href=
     *      "http://msdn.microsoft.com/en-us/library/windows/desktop/ms694359%28v=vs.85%29.aspx">MSDN</a>
     * 
     */
     ...
     * @return {@link WinError#S_OK S_OK} if the COM library and additional
     *         functionality were initialized successfully on this apartment.
     *         <p>
     *         {@link WinError#S_FALSE S_FALSE} if the COM library is already
     *         initialized on this apartment.
     *         <p>
     *         {@link WinError#OLE_E_WRONGCOMPOBJ OLE_E_WRONGCOMPOBJ} if the
     *         versions of COMPOBJ.DLL and OLE2.DLL on your machine are
     *         incompatible with each other.
     *         <p>
     *         {@link WinError#RPC_E_CHANGED_MODE RPC_E_CHANGED_MODE} if a
     *         previous call to CoInitializeEx specified the concurrency model
     *         for this apartment as multithread apartment (MTA). If running
     ...
     *
     * Remarks: Call OleUninitialize on application shutdown, as the last COM
     * library call, if the apartment was initialized with a call to
     ...
     *         <p>
     *         {@link WinError#CLIPBRD_E_CANT_OPEN CLIPBRD_E_CANT_OPEN} The
     *         Windows OpenClipboard function used within OleFlushClipboard
     *         failed.
     *         <p>
     *         {@link WinError#CLIPBRD_E_CANT_CLOSE CLIPBRD_E_CANT_CLOSE} The
     *         Windows CloseClipboard function used within OleFlushClipboard
     *         failed.
     *         <p>
     *         <b>Remarks</b>
     *         <p>
     *         OleFlushClipboard renders the data from a data object onto the
     *         clipboard and releases the IDataObject pointer to the data
     *         object. While the application that put the data object on the
     *         clipboard is running, the clipboard holds only a pointer to the
     *         data object, thus saving memory. If you are writing an
     *         application that acts as the source of a clipboard operation, you
     *         can call the OleFlushClipboard function when your application is
     *         closed, such as when the user exits from your application.
     *         Calling OleFlushClipboard enables pasting and paste-linking of
     *         OLE objects after application shutdown. Before calling
     *         OleFlushClipboard, you can easily determine if your data is still
     *         on the clipboard with a call to the OleIsCurrentClipboard
     *         function.
     *
     *         OleFlushClipboard leaves all formats offered by the data transfer
     *         object, including the OLE 1 compatibility formats, on the
     *         clipboard so they are available after application shutdown. In
     *         addition to OLE 1 compatibility formats, these include all
     *         formats offered on a global handle medium (all except for
     *         TYMED_FILE) and formatted with a null target device. For example,
     *         if a data-source application offers a particular clipboard format
     *         (say cfFOO) on an IStorage object, and calls the
     *         OleFlushClipboard function, the storage object is copied into
     *         memory and the hglobal memory handle is put on the clipboard.
     *
     *         To retrieve the information on the clipboard, you can call the
     *         OleGetClipboard function from another application, which creates
     *         a default data object, and the hglobal from the clipboard again
     *         becomes a storage object. Furthermore, the FORMATETC enumerator
     *         and the IDataObject::QueryGetData method would all correctly
     *         indicate that the original clipboard format (cfFOO) is again
     *         available on a TYMED_ISTORAGE.
     *
     *         To empty the clipboard, call the OleSetClipboard function
     *         specifying a null value for its parameter. The application should
     *         call this when it closes if there is no need to leave data on the
     *         clipboard after shutdown, or if data will be placed on the
     *         clipboard using the standard Windows clipboard functions.
     */
     ...
     * @param pUnknown
     *            [in] Pointer to the
     *            {@link com.sun.jna.platform.win32.COM.IUnknown IUnknown}
     *            interface on the object, with which it will query for a
     *            pointer to the IRunnableObject interface, and then call its
     *            Run method.
     * @return This function returns on success. Other possible values include
     *         the following.
     *         <p>
     ...
     *         implementation of OleRun was changed in OLE 2.01 to coincide with
     *         the publication of the IRunnableObject interface. You can use
     *         OleRun and IRunnableObject::Run interchangeably. OleRun queries
     *         the object for a pointer to IRunnableObject. If successful, the
