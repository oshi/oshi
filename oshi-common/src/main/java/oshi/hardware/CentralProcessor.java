...
public interface CentralProcessor {
     ...
     * {@code oshi.os.mac.cpu.frequency.ioreport} to {@code true} instead returns, per core, the average of its
     * cluster's frequencies weighted by how long the core spent at each of them since the previous call. That measure
     * needs an interval, so the first call establishes the baseline and returns the nominal frequencies, as does any
     * call the operating system cannot supply residency for.
     *
