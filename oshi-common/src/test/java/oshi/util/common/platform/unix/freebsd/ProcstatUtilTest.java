...
class ProcstatUtilTest {
    ...
    void testProcstatLive() {
        if (PlatformEnum.getCurrentPlatform().equals(PlatformEnum.FREEBSD)) {
            ...
            // or requiring JDK 9+ ProcessHandle (oshi-common targets JDK 8 bytecode).
            int pid = ParseUtil.parseIntOrDefault(ManagementFactory.getRuntimeMXBean().getName().split("@", 2)[0], 0);
