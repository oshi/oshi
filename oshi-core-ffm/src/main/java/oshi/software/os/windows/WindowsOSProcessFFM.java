...
import static oshi.ffm.platform.windows.Advapi32FFM.TokenPrimaryGroup;
import static oshi.ffm.platform.windows.Advapi32FFM.TokenUser;
import static oshi.ffm.platform.windows.NtDllFFM.PBI_PEB_BASE_ADDRESS_OFFSET;
import static oshi.ffm.platform.windows.NtDllFFM.PEB;
import static oshi.ffm.platform.windows.NtDllFFM.PEB_PROCESS_PARAMETERS_OFFSET;
import static oshi.ffm.platform.windows.NtDllFFM.PROCESS_BASIC_INFORMATION;
import static oshi.ffm.platform.windows.NtDllFFM.PROCESS_BASIC_INFORMATION_STRUCT;
import static oshi.ffm.platform.windows.NtDllFFM.RTL_USER_PROCESS_PARAMETERS;
import static oshi.ffm.platform.windows.NtDllFFM.UNICODE_STRING;
import static oshi.ffm.platform.windows.NtDllFFM.UPP_COMMAND_LINE_OFFSET;
import static oshi.ffm.platform.windows.NtDllFFM.UPP_CURRENT_DIRECTORY_OFFSET;
import static oshi.ffm.platform.windows.NtDllFFM.UPP_ENVIRONMENT_OFFSET;
import static oshi.ffm.platform.windows.NtDllFFM.UPP_ENVIRONMENT_SIZE_OFFSET;
import static oshi.ffm.platform.windows.NtDllFFM.readUnicodeString;
import static oshi.ffm.platform.windows.WinNTFFM.PROCESS_QUERY_INFORMATION;
...
import oshi.ffm.platform.windows.NtDllFFM;
import oshi.ffm.platform.windows.Shell32FFM;
            ...
            try (NativeHandle pHandle = NativeHandle.of(pHandleOpt.get(), Kernel32FFM::CloseHandle);
                ...
                if (Advapi32FFM.OpenProcessToken(pHandle.get(), TOKEN_QUERY, hTokenPtr)) {
                    try (NativeHandle hToken = NativeHandle.of(hTokenPtr.get(ADDRESS, 0), Kernel32FFM::CloseHandle)) {
                        ...
                        return token == null ? null : getTokenAccountInfo(token, tokenInformationClass, arena);
                    }
                }
    ...
    protected Triplet<String, String, Map<String, String>> queryCwdCommandlineEnvironment() {
        ...
        if (hOpt.isPresent()) {
            try (NativeHandle h = NativeHandle.of(hOpt.get(), Kernel32FFM::CloseHandle);
                ...
                if (process != null && WindowsOperatingSystemFFM.isX86() == isWow(process, arena)) {
