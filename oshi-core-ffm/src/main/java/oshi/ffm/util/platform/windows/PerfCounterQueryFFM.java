...
public final class PerfCounterQueryFFM {
    ...
    public static <T extends Enum<T> & PdhCounterProperty> Map<T, Long> queryValuesFromWMI(Class<T> propertyEnum,
        ...
        List<Map<T, Long>> rows = Objects.requireNonNull(WmiQueryHandlerFFM.createInstance()).queryWMI(perfWmiClass,
                null, () -> new EnumMap<T, Long>(propertyEnum), (pObject, arena, row) -> {
                    for (T prop : props) {
